...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	/**
	 * 当前创建的 bean 的名称，用于隐式依赖注册
	 * 从用户指定的供应商回调触发的 getBean 等调用.
	 */
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 确保此时真正解析了 bean 类，
		// 并在动态解析的类无法存储在共享合并 bean 定义中的情况下克隆 bean 定义。
		...
		try {
			// InstantiationAwareBeanPostProcessor 的 postProcessBeforeInstantiation方法
			// 如果bean配置了 PostProcessor ，那么返回一个proxy  通常用于抑制特定目标bean的默认实例化
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		// 装饰器模式  声明一个 BeanWrapper 用来持有创建出来的bean 对象
		BeanWrapper instanceWrapper = null;
		if (mbd.isSingleton()) {
			//如果是单例的，先不缓存中同名的bean清除
		...
		// 这里完成了具体的实例化环节 但是此时的bean对象是一个半成品 并且没有注入依赖
		...
		// 急切地缓存单例以解决循环引用
		// 即使被 BeanFactoryAware 等生命周期接口触发.
		...
		if (earlySingletonExposure) {
			...
			//  三级缓存 放入             getEarlyBeanReference  可以解决循环依赖的问题
	...
	/**
	 * Obtain a reference for early access to the specified bean, 获取早期访问指定 bean 的参考
	 * typically for the purpose of resolving a circular reference. 通常用于解决循环引用
	 * @param beanName the name of the bean (for error handling purposes)
	 * @param mbd the merged bean definition for the bean
	 ...
	 *	解决指定bean是否有pre-instantiation快捷方式.
	 * @param beanName the name of the bean
	 * @param mbd the bean definition for the bean
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		if (instanceSupplier != null) {
			// 支持手动去创建beanDefination  并且利用回调的方式去创建对象 ？
	...
	/**
	 * 重写以便将当前创建的 bean 隐式注册为依赖于在 {@link Supplier} 回调期间以编程方式检索的其他 bean.
	 * @since 5.0
	 * @see #obtainFromSupplier
	 */
	@Override
	 ...
	 * 用于调试消息；未在功能上使用.
	 * @param mbd 通过自动装配更新的 bean 定义
	 * @param bw 我们可以从中获取有关 bean 的信息的 BeanWrapper
	 * @param pvs 用于注册连接对象的 PropertyValues
	 */
	 ...
	 */
	// 从给定BeanRapper提取一组过滤后的PropertyDescriptor，排除忽略的依赖类型或在忽略的依赖接口上定义的属性。
	...
	// 执行相关性检查，以确保已设置所有公开的属性，如果需要的话。依赖性检查可以是对象（协作bean），
	 ...
	 */
	// 应用给定的属性值，解析任何运行时引用，到这个Bean工厂的其他bean。必须使用深度复制，因此我们不要永久修改此属性。
	...
	protected void invokeInitMethods(String beanName, Object bean, @Nullable RootBeanDefinition mbd)
		...
		//  如果bean 是 InitializingBean 的 实例
